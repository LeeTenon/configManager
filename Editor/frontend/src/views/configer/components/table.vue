                ...
                <div v-if="isNil(scope.row.children)" style="display: flex; align-items: center">
                    <div v-if="isNil(scope.row.common) || scope.row.force" class="value-box">
                        <el-select v-if="scope.row.type == 'enum'" class="common-value-input" placeholder="Select"
                            ...
                            <el-option v-for="(value,key) in enums![scope.row.spec]" :key="value" :label="key"
                                :value="key" />
                        </el-select>
                    ...
                    <div v-else class="value-box">
                        ...
                        <el-input-number v-else-if="scope.row.type == 'number'" v-model="scope.row.common.value"
                            size="small" disabled class="common-value-input" />
                        <el-input v-else v-model="scope.row.common.value" size="small" disabled />
                        <el-button @click="scope.row.force = !scope.row.force" size="small" style="margin-left: 15px;">
                            <span>修改</span>
                        </el-button>
                    </div>
                </div>
