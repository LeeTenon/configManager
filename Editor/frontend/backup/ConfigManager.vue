          ...
          <el-table-column prop="value" label="值" style="width: 50%">
            <template #default="scope">
              <div v-if="scope.row.children.length == 0" style="display: flex; align-items: center">
                <div v-if="isUndefined(scope.row.common) || !isZero(scope.row.value) || scope.row.force" class="value-box">
                  ...
                  <el-button v-if="!isNull(scope.row.common)"
                    @click="scope.row.value=null; scope.row.force = !scope.row.force" size="small"
...
function toTree(data: any): any { //转换树形
  ...
  // 关联common配置
  for (let commonMode in tree['CommonConfig']) {
    for (let service in tree) {
      if (service != 'CommonConfig' && commonMode in tree[service]) {
        setCommon(tree['CommonConfig'][commonMode], tree[service][commonMode])
      }
    }
  }
  return tree
 ...
 * @function 读取common配置
 */
function setCommon(src: Node[], dst: Node[]) {
...
function addConfig(service: any, name: any) {
  ...
  if (addTag.value == "empty") {
    if (service == 'CommonConfig') {
      ...
      treeData.value[service][name] = objToTree(template[service])
    }
  } else {
  ...
  // 绑定common配置
  if (service == 'CommonConfig') {
    for (let s in treeData) {
      if (name in treeData[s] && s != service) {
        setCommon(treeData[service][name], treeData[s][name])
      }
    }
  } else {
    if (name in treeData['CommonConfig']) {
      setCommon(treeData['CommonConfig'][name], treeData[service][name])
    }
  }
  //加入导出mode列表
  let isValid = false
